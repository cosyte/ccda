...
- `requiredSectionKeys("ccd")` and `missingRequiredSections("ccd", ...)` now include `socialHistory` and `vitalSigns`. A parsed CCD that lacks either now raises a `REQUIRED_SECTION_MISSING` warning it previously did not. This is a warning, never a fatal, so nothing stops parsing.
