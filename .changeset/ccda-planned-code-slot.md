...
Code-system check a planned medication's drug exactly as a performed one's is checked: an empty
`<code/>` on a planned drug now draws `MISSING_CODE_VALUE`, which no vendor profile can quiet.

Before this, such a planned medication reached a consumer carrying only a profile-quietable warning.
