...
describe("AC-9: a run leaves no fetched artifact visible to this repository's own gates", () => {
  it("reports the same set of paths from git status after the run as before it", async () => {
  ...
  }, // BUDGET, argued from what this test does rather than from a machine. It fetches four
  // artifacts through the doubled network, compiles a Schematron, builds and validates four
  // documents through libxml2 in WebAssembly, and round-trips a corpus document. The WASM
  // module is instantiated once per validated document and is the dominant cost; measured on
  // this suite it runs in well under a second, and the ceiling is set an order of magnitude
  // above that so a loaded box does not turn a pass into a false red. There is deliberately
  // no global testTimeout in vitest.config.ts and this number does not belong there.
  20_000);

  it("leaves pnpm phi-scan green over the whole working tree after the run", () => {
  ...
  }, // BUDGET: one process start plus a full-tree scan of a few hundred files. The repository's
  // own scanner suite sizes a `node` start at roughly 200 ms and this adds the walk; 20
  // seconds is the same order-of-magnitude headroom as the case above, for the same reason.
  20_000);
});
