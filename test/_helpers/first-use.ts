 ...
 * the repository root, with each specifier in `paths` resolved to the single source file the
 * bundler compiles into that published entry point and its types. Only the example's own errors
 * are returned.
 */
export function compileErrors(
  ...
  const options: ts.CompilerOptions = {
    ...
    paths: Object.fromEntries(Object.entries(paths).map(([pkg, entry]) => [pkg, [entry]])),
  };
...
/** Every backtick-quoted literal in a snippet that holds a C-CDA document. */
export function documentLiterals(code: string): string[] {
  const literals: string[] = [];
  for (const match of code.matchAll(/`([^`]*<ClinicalDocument\b[^`]*)`/g)) {
...
/**
 * Every fenced block whose info string carries the `runnable` tag, with the 1-based line of its
 * opening fence and its language token. A fence may be indented (inside a list item), and closes on
 * the same marker at the same indentation, which is how the snippet harness reads it too.
 */
export function runnableTaggedFences(markdown: string): { line: number; lang: string }[] {
  const out: { line: number; lang: string }[] = [];
  let open: string | undefined;
  markdown.split("\n").forEach((text, index) => {
    if (open !== undefined) {
      if (text.trimEnd() === open) open = undefined;
      return;
    }
    const opened = /^(\s*)(`{3,}|~{3,})\s*(.*)$/.exec(text);
    if (opened === null) return;
    open = `${opened[1] ?? ""}${opened[2] ?? "```"}`;
    const tokens = (opened[3] ?? "").trim().split(/\s+/).filter(Boolean);
