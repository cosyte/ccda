...
- cbed1a0: feat: measure conformance against the normative artifacts, and fix what the measurement found
...
- cbed1a0: **This is 0.1.0, the first release whose public API we treat as settled.**
...
- cbed1a0: required sections: every document type now reports how far it was verified
...
- cbed1a0: feat: report a coded value that sits outside the value set its slot is bound to
...
- cbed1a0: required sections: a SHALL warning fires exactly where the normative source says it should
...
- cbed1a0: version stamps: a document says which C-CDA release it was written for
...
- cbed1a0: subject override: an entry is never read as the patient's unless it is
...
- cbed1a0: A parsed document now says who authored it: the parser reads the header participations.
...
- cbed1a0: Build an inpatient Discharge Summary, the third of the twelve document types
...
- cbed1a0: Add the shared `toObject` / `toISO` / `toDate` conversion surface for a parsed `TS`
...
- cbed1a0: docs: cover the profile, terminology and conformance surfaces, and gate the bundle
...
- cbed1a0: Bring this repo to the shared package baseline: install hardening, the required
...
- cbed1a0: README: standardize the page, and make its usage example a test
...
- cbed1a0: phi-scan: a target enumerated and never read refuses the scan
