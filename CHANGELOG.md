...
value="12"/>` returned a score of 12 with no warning. The `integer` observation value now carries
  ...
  - **A medication under `manufacturedLabeledDrug` no longer loses its drug
    ...
    `MEDICATION_PRODUCT_ARM_UNEXPECTED` is deliberately **not** safety-critical: the drug is present
    and fully checked, so this flags known vendor shape rather than lost clinical data and a profile
    may defensibly tolerate it. Whether the C-CDA template _forbids_ the alternate arm is a normative
